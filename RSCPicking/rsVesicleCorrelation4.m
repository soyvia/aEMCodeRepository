function [ampImg, mxInds, mxNCCImg, mxValsV, mxRso, localVar]=rsVesicleCorrelation4(m,mi,vesIndex,mbnOffset,maskRadii,angleInds,eigenSet)
...
msks=([msko(pix) mski(pix)]>.5)';  % 2 x npix binary mask
nccPixMasked=nccPix.*repmat(msks,nHemi*nGamma,1); % nRefs x npix

msksV=mskv(pix)';
% cimgV=cimg.*repmat(msksV,nRefs,1)./paArray;  % whole-vesicle cross-correlation
nccVPixMasked=nccPix.*repmat(msksV,nRefs,1);  % whole-vesicle cross-correlation

% find the maximum of the NCC at each active pixel
[mxNCC, mxi]=max(nccPixMasked);  % 1 x nPix : max over nRefs

% normalize by the chosen optimum match to get estimated amplitude
denormNCC=mxNCC'.*pixNorm;
ampPix=zeros(npix,1);
for i=1:npix
...
[mxNCCV, mxiV]=max(nccVPixMasked);
denormNCCV=mxNCCV'.*pixNorm;
ampVPix=zeros(npix,1);
for i=1:npix
    ampVPix(i)=denormNCCV(i)/(s0*paArray(mxiV(i),i));
end;
