function [out1,out2,out3] = Myginput(arg1,pointerType)
% function [x,y,button] = Myginput(arg1,pointerType)
% Like the standard ginput, but allows the pointer to be specified.
% The default is a standard small crosshair.
if nargin<2
...
%     Create my pointer here
    if strcmpi(pointerType,'square')
        ...
        ptrMap(range,1)=1;
        ptrMap(range,pn-1)=1;
        set(fig,'pointer','custom','pointershapecdata',ptrMap,...
            'pointershapehotspot',[1 1]*(pn/2-1));
    else
        set(fig,'Pointer',pointerType);
    end;
