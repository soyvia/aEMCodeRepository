function [m, pixA, doses]=meReadImagesNorm(fnames, cpe, flipFirst,overridePixA, weights)
% function [m pixA doses]=meReadImagesNorm(fnames, cpe, flipFirst,overridePixA, weights)
...
% edges (square window, n/16 pixels) and normalized to unity contrast.
...
for findex=1:nfiles
    if weights(findex)
        ...
        if numel(name)>0
            ...
            dose=sum(m1(:))/sum(win(:)); % mean value of windowed image
            % remove the mean and store as e / A^2
            m(:,:,findex)=single(m1/dose-win);  % Normalized, zero-mean image
            doses(findex)=dose;
        end;
    end;
end;
