...
% optimal SSNR.  The effective CTF is returned in the 2D array effctz.  the
% individual aligned images are returned, if desired, in the stack mts.
% Those images are downsampled but are not filtered except by a circular
% mask if given.  Thus mc = sum_i real( ifftn( fftn(mts(:,:,i)).*
% ifftshift(coeffz(:,:,i))))
% The individual ctfs of the mts (with dose-dependent decay) are returned as dctfs.
...
if nargout>2
    mts=single(zeros([nd0 nim]));  % Array to receive aligned images
end;
